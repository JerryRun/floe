...
func TestMemoryStoreEncryptsPersistsAndSearches(t *testing.T) {
	directory := t.TempDir()
	...
	data, err := os.ReadFile(filepath.Join(directory, "memories.json"))
	if err != nil {
		t.Fatal(err)
	}
	for _, secret := range []string{"测试数据库账号", "192.168.8.22", "very-secret-value"} {
		if bytes.Contains(data, []byte(secret)) {
			t.Fatalf("encrypted store contains plaintext %q", secret)
		}
	}
...
func TestMemorySettingsCopiesAndSwitchesStore(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dataDir, "memories.json")); err != nil {
...
func TestCopyMemoryStoreRejectsExistingTarget(t *testing.T) {
	...
	target := t.TempDir()
	if err := os.WriteFile(filepath.Join(target, "memory.key"), []byte("existing"), 0o600); err != nil {
		t.Fatal(err)
	}
