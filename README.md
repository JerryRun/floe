...
- SFTP 上传、下载以及 FTP 下载支持多路并发。标准 FTP 不保证对同一文件的
  随机并发写入，因此 FTP 上传按分块顺序写入并逐块回读校验，避免先零填充整文件
  带来的双倍上传流量。
